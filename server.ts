...
async function startServer() {
  ...
  // Lazy initialize GoogleGenAI with User-Agent header as required
  ...
  // Endpoint para Extração Inteligente de Nota Fiscal (DANFE)
  app.post('/api/extrair-nf', async (req, res) => {
    try {
      ...
      if (!process.env.GEMINI_API_KEY) {
        return res.status(503).json({
          error: 'Chave GEMINI_API_KEY não configurada no servidor. Use a extração local.',
        });
      }
...
`;
      ...
      console.error('Erro ao processar extração no Gemini:', err);
